...
{
    /// <summary>
    /// Claude 配置页签 ViewModel
    /// </summary>
    public class ClaudeTabViewModel : ReactiveObject
    {
        ...
        public ClaudeTabViewModel(ConfigurationService configService)
        {
        ...
        public void RefreshConfigurations()
        {
            try
            {
                ...
                var configs = string.IsNullOrWhiteSpace(SearchText)
                    ? _configService.GetConfigurationsByProvider(AIProvider.Claude)
                    : _configService.SearchConfigurations(AIProvider.Claude, SearchText);
            ...
            {
                Console.WriteLine($"刷新Claude配置列表失败: {ex.Message}");
            }
        }
