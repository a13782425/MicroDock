...
namespace FileToolPlugin
{
    public class FileToolPlugin : BaseMicroDockPlugin
    {
        ...
        [PluginTool("file.read", Description = "读取一个文件并将其内容以 Base64 字符串形式返回", ReturnDescription = "Base64 编码的文件内容")]
        public async Task<string> ReadFile(
            [PluginToolParameter("path", Description = "文件的绝对路径")] string path)
        {
            return await RunConsoleCommand("read", path);
        }

        [PluginTool("file.write", Description = "将 Base64 格式的内容写入文件中", ReturnDescription = "成功消息或错误信息")]
        public async Task<string> WriteFile(
            [PluginToolParameter("path", Description = "文件的绝对路径")] string path,
            [PluginToolParameter("content", Description = "Base64 编码的文件内容")] string content)
        {
