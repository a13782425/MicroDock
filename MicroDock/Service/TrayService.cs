...
/// </summary>
[AutoRegister]
public class TrayService : IMicroService
{
    ...
    Task IMicroService.OnAfterSplashScreen()
    {
        if (_trayIcon != null)
        {
            LogWarning("TrayService 已经初始化", DEFAULT_LOG_TAG);
        ...
        {
            LogError("加载托盘图标失败", DEFAULT_LOG_TAG, ex);
        }
        ...
        _trayIcon.Clicked += (sender, args) =>
        {
            ...
            {
                if (MicroMainWindow.IsVisible)
                {
                    MicroMainWindow.Hide();
                }
                else
                {
                    MicroMainWindow.Show();
                    // 如果窗口处于最小化状态，需要还原
                    if (MicroMainWindow.WindowState == WindowState.Minimized)
                    {
                        MicroMainWindow.WindowState = WindowState.Normal;
                    }

                    MicroMainWindow.Activate();
                    // 2. 智能置顶逻辑
                    // 只有当窗口当前【不是】置顶状态时，才执行"临时置顶"策略
                    // 这样既能把窗口拉到最前，又不会误关掉用户开启的"始终置顶"
                    if (!MicroMainWindow.Topmost)
                    {
                        MicroMainWindow.Topmost = true;
                        ...
                        Avalonia.Threading.Dispatcher.UIThread.Post(() =>
                        {
                            ...
                            if (MicroMainWindow != null && !ServiceLocator.Get<TopMostService>().IsEnabled)
                            {
                                // 注意：这里最好结合你的 SettingDB 或 TopMostService 状态来判断
                                // 如果不想依赖 Service，最简单的做法是直接还原:
                                MicroMainWindow.Topmost = false;
                            }
                            else
                            {
        ...
        LogInformation("TrayService 初始化完成", DEFAULT_LOG_TAG);
        return Task.CompletedTask;
    }
