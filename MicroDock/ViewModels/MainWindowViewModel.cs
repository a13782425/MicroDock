...
namespace MicroDock.ViewModels
{
    public class MainWindowViewModel : ViewModelBase
    {
        ...
        public MainWindowViewModel()
        {
            ...
            // 添加固定的"应用"页签（不可关闭）
            Tabs.Add(new TabItemModel("应用", TabType.Application, isClosable: false));
            
            // 加载插件页签
            LoadPluginTabs();
            
            // 添加固定的"设置"页签（不可关闭）
            Tabs.Add(new TabItemModel("设置", TabType.Settings, isClosable: false));
        ...
        private void ExecuteAddCustomTab()
        {
            ...
            TabItemModel newTab = new TabItemModel($"自定义 {customTabCount}", TabType.Custom, isClosable: true);
