...
2. **版本兼容性**：
   ```csharp
   // Data 目录中的数据可能来自旧版本，需要做兼容处理
   public override void OnInit()
   {
       MigrateUserDataIfNeeded(); // 检查并升级旧版本数据格式
   }
   
   private void MigrateUserDataIfNeeded()
   {
       // 检查数据版本
       var dataVersion = GetValue("data_version") ?? "1.0.0";
       
       if (Version.Parse(dataVersion) < Version.Parse("2.0.0"))
       {
           // 升级旧版本数据格式
           MigrateFrom1To2();
           SetValue("data_version", "2.0.0");
       }
   }
   ```
...
Plugins/
└── MyPlugin/
    ├── Config/      # 配置目录（GetConfigPath()）
    ├── Data/        # 数据目录（GetPluginDataPath()）
    └── MyPlugin.dll
```
