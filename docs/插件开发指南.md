...
{
    string TabName { get; }        // 标签页名称
    object TabContent { get; }     // 标签页内容（通常是 UserControl）
    IconSource TabIcon { get; }    // 标签页图标
}
```

### IMicroActionsProvider（操作提供者接口）
...
{
    IEnumerable<IAction> GetActions();  // 返回可用的操作列表
}
```
...
public override void OnAllPluginsLoaded()
{
    ...
    // 1. 调用全局工具（调用第一个匹配的）
    var result1 = await CallToolAsync("string.reverse", new Dictionary<string, object>
    {
        ["input"] = "Hello World"
    });
    
    // 2. 指定插件名调用
    var result2 = await CallToolAsync("math.add", new Dictionary<string, object>
    {
        ["a"] = 10,
        ["b"] = 20
    ...
    var result3 = await CallToolAsync("data.filter", new Dictionary<string, object>
    {
        ["items"] = items,  // 会自动序列化为 JSON
        ["minValue"] = 10
    });
}
```
...
1. **类型匹配**: 如果参数类型已经匹配，直接使用
2. **基本类型转换**: 使用 `Convert.ChangeType`
3. **复杂类型**: 通过 JSON 序列化/反序列化转换

```csharp
// 调用时：
await CallToolAsync("tool.name", new Dictionary<string, object>
{
    ["intParam"] = 42,  // int -> int (直接)
    ["stringParam"] = "hello",  // string -> string (直接)
    ["complexParam"] = myObject  // MyObject -> JSON -> MyObject (序列化)
});
```
